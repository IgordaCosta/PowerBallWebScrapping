...
function StartALL  (){
    ...
    let DataBaseType
    function GetDataFromSitefunction (DataBaseType,callback){
    ...
    const GetDataFromSitePromise = opts => {
        const promise = new Promise((resolve, reject) => {
            GetDataFromSitefunction (DataBaseType,
            success => {
              resolve(success);
            },
...
TableExists=db.get("SELECT name FROM sqlite_master WHERE type='table' AND name="+"'"+OldDatabaseName+"'", function(error, row) {
    let TableExists =false
		if (row !== undefined) {
            ...
            GetDataFromSitePromise(NewDatabase).then(GetDataFromSitePromise(OldDatabase))
            // GetDataFromSitePromise(NewDatabase).then(PowerBallWebScrape.GetDataFromSite(OldDatabase))
